...
import { NavLinkType } from "@/src/models/nav";
import {
  motion,
  MotionValue,
  useSpring,
...
const NavLink = ({
  ...
  return (
    ...
    //   {link.icon}
    // </motion.li>
    <li className="relative group" ref={ref} onClick={() => handleClick(link.href)} aria-label={link.label}>
      <div className="absolute w-0 h-0.5 bg-primary -bottom-1 rounded-full group-hover:w-20"></div>
      {link.label}
    </li>
  );
};
